...
Keypoints_songs = struct2cell(load('C:\Users\saravanos\Desktop\audioFingerprinting_NEW\Centroids_DBs\DB4\Time\Songs\128_256\Frames\Keypoints_DominantAtomsWithConstraints.mat'));
...
audiowrite('C:\Users\saravanos\Desktop\audioFingerprinting_NEW\NoisyAudioClips\audioClip_12_80.wav', audioClip6, 44100)
...
audiowrite('C:\Users\saravanos\Desktop\audioFingerprinting_NEW\NoisyAudioClips\audioClip_12_160.wav', audioClip7, 44100)
...
audiowrite('C:\Users\saravanos\Desktop\audioFingerprinting_NEW\NoisyAudioClips\audioClip_12_80_160.wav', audioClip8, 44100)
...
[~, ~, keypoints5, ~, ~]= CreateTheAudioClipsFingerprint_Frames(AudioClip5, D); 
numberOfClipsFrames5 = size(keypoints5,1); 

[~, ~, keypoints6, ~, ~]= CreateTheAudioClipsFingerprint_Frames(AudioClip6, D); 
numberOfClipsFrames6 = size(keypoints6,1); 
keypoints6 = keypoints6(:,3);


[~, ~, keypoints7, ~, ~]= CreateTheAudioClipsFingerprint_Frames(AudioClip7, D); 
numberOfClipsFrames7 = size(keypoints7,1); 


[~, ~, keypoints8, ~, ~]= CreateTheAudioClipsFingerprint_Frames(AudioClip8, D); 
numberOfClipsFrames8 = size(keypoints8,1); 
...
pearsonCoeff5 = zeros(size(Keypoints_songs,1), 1); 
pearsonCoeff6 = zeros(size(Keypoints_songs,1), 1); 
pearsonCoeff8 = zeros(size(Keypoints_songs,1), 1); 
...
for i = 1:size(Keypoints_songs,1)
    ...
    for j = 1:numberOfWindows
        ...
        pearsons(j) = corr(keypoints(:,3), tempKeypoints);
        euclideanDistances(j) = sqrt(sum((keypoints(:,3)-tempKeypoints).^2)); 
    end 
...
for i = 1: size(Keypoints_songs,1)
    ...
    for j = 1:numberOfWindows
        ...
        pearson5 = corr(keypoints5(:,3), tempKeypoints);
        pearson6 = corr(keypoints6(:,3), tempKeypoints);
        
       pearson7 = corr(keypoints7(:,3), tempKeypoints);
        
        pearson8 = corr(keypoints8(:,3), tempKeypoints);
...
%        hammings(j)= hamm;
 ...
 %       hammings2(j)= hamm2;
  ...
  %      hammings3(j)= hamm3;
        ...
        pearsons5(j)=pearson5;
        pearsons6(j)=pearson6;
          pearsons7(j)=pearson7;
        
        pearsons8(j)=pearson8;
   ...
   % hammingDist4(i)= min(hammings4); 
    ...
    pearsonCoeff5(i)= max( pearsons5);
    pearsonCoeff6(i)= max( pearsons6);
    pearsonCoeff7(i)= max( pearsons7);
    
    pearsonCoeff8(i)= max( pearsons8);
...
[minValue_p5, minIdx_p5] = max(pearsonCoeff5);
